...
def process_csv_directory(csv_directory, output_file, number_of_prev_hours=24):
    ...
    start_day, start_month = 5, 1
    end_day, end_month = 19, 1

    start_time = pd.Timestamp(year=2017, month=start_month, day=start_day)
    end_time = pd.Timestamp(year=2017, month=end_month, day=end_day)

    total_files = 0
    total_rows_collected = 0

    for file_name in tqdm(os.listdir(csv_directory)):
        if not file_name.endswith('.csv') or '2017' not in file_name:
            continue
...
process_csv_directory(
    ...
    output_file= r"C:\Users\hadar\Desktop\אוניברסיטה\פרויקט גמר\relevant_directories\testing\data_2018_train.csv",
    number_of_prev_hours=4
)
